 ...
 */
const ProfileInfo = ({ style = "", messageCount, profileImageURLs, hasBackgroundImage = false }) => {
  // li 요소에 이미 3명이 표시되었으므로 writerCount에서 3을 뺀 값을 변수에 저장
  const messageCountMinusThree = () => (messageCount <= 3 ? 0 : messageCount - 3);

  const containerClassNames = clsx(styles.profileInfoContainer, styles[style]);
  const listContainerClassNames = clsx(styles.profileImageListContainer, styles[style]);
  const listItemClassNames = clsx(styles.profileImageWrapper, styles[style]);
  const descriptionClassNames = clsx(styles.description, styles[style], hasBackgroundImage && styles.white);
      ...
      <ul className={listContainerClassNames}>
        {profileImageURLs.slice(0, 3).map((url, index) => (
          <li key={index} className={listItemClassNames}>
            {profileImageURLs.length >= index + 1 && messageCount >= index + 1 && (
              <img className={styles.profileImage} src={url} alt={`프로필 이미지 ${index + 1}`}></img>
            )}
          </li>
        ))}
